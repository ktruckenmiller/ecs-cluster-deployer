...
LATEST_AMI=$(aws ssm get-parameter --name /aws/service/ecs/optimized-ami/amazon-linux/recommended/image_id --region ${AWS_DEFAULT_REGION} --query "Parameter.Value")
...
if [ "${FOUND_AMI}" = "None" ]; then
  ...
  echo "Validating Packer tempcdlate"
